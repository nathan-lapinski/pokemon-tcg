...
/**
 * This is a test service for experimenting with ideas. Many of the features here, such as generating packs,
 * will go somewhere else, such as the api layer. This class will not be included in production builds
 */
@Injectable({
    providedIn: 'root'
  })
export class TestPokemonDataService {
    public getTestData(): Observable<any> {
        return of(this.testData);
    }

    public getPack(): Observable<any> {
        const pack: any[] = this.generatePack();
        return of(pack);
    }

    // TODO: Ineffecient. Let the api layer or DB layer do this.
    private generatePack(): any[] {
        let rares = [];
        let uncommons = [];
        let commons = [];
        let energy = [];

        rares = this.testData.filter(card => card.rarity === 'Rare');
        ...
        return [].concat(rares, uncommons, commons, energy);
    }

    // all of the cards from base set 1
    private testData = [
        {
          "id": "base1-1",
          "name": "Alakazam",
